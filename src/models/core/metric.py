...
def drqa_start_accuracy(y_true, y_pred):
    return __drqa_accuracy(y_true[:, :, 0], y_pred[:, :, 0])


def drqa_end_accuracy(y_true, y_pred):
...
def drqa_tot_accuracy(y_true, y_pred):

    def _aggregate(s_acc, e_acc):
        return (s_acc + e_acc) / 2

    s_acc = drqa_start_accuracy(y_true, y_pred)
    e_acc = drqa_end_accuracy(y_true, y_pred)
